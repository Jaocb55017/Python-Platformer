...
class Game:
    ...
    def new(self):
        ...
        for rock in rock_array:
            Rocks(self, *rock)
        self.mob_timer = 0
        #pygame.mixer.music.load(path.join(self.sound_dir, 'something')) ##################### Main Music
    ...
    def update(self):
        ...
        coin_hit_spike = pygame.sprite.groupcollide(self.spikes, self.coins, False, True)
        powerup_hit_spike = pygame.sprite.groupcollide(self.spikes, self.powerups, False, True)
        poweruphit = pygame.sprite.spritecollide(self.player, self.powerups, False, pygame.sprite.collide_mask)
        ...
        if self.player.rect.right >= 300:
            ...
            for powerup in self.powerups:
                powerup.rect.x -= max(abs(self.player.vel.x), 5)
    ...
    def show_go_game_display(self):
        ...
        self.game_display.fill(background_colour)
        self.text_on_screen("GAME OVER", 48, white, game_width - game_width + 120, game_height / 8 - 50)
        self.text_on_screen("Score: " + str(self.score), 22, white, game_width / 2, 15)
        self.text_on_screen("Answer with the appropriate key to play again.", 32, white, game_width / 2, 500)
        self.text_on_screen("Which following statement is true for a Network Bridge?", 32, white, game_width / 2, 150)
        self.text_on_screen("A: A bridge is more as capable as a Router - but it is more expensive.", 22, white, game_width / 2, 200)
        self.text_on_screen("B: A bridge is bad and shouldnt be used.", 22, white, game_width / 2, 250)
        self.text_on_screen("C: A bridge is not as capable as a Router - but it is less expensive.", 22, white, game_width / 2, 300)
        ...
        if self.score > self.highscore:
            ...
            self.text_on_screen("High Score: " + str(self.highscore), 22, white, game_width / 2, 30)
        pygame.display.flip()
        self.any_key_press()
        pygame.mixer.music.fadeout(500)
